...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        ...
        for (NSDictionary *pair in pairs) {
            ...
            if (!orion_qwen9b_lora_train_smoke1(blob_dir, &trainer, inputToken, targetToken, &result)) {
                fprintf(stderr, "FAIL: pair step failed at step %d for sample %s\n", steps_completed + 1, sampleId.UTF8String);
                orion_qwen9b_lora_trainer_free(&trainer);
                orion_qwen35_manifest_free(manifest);
                return 1;
            }
        ...
        if (steps_completed == 0) {
            fprintf(stderr, "FAIL: zero steps completed\n");
            orion_qwen9b_lora_trainer_free(&trainer);
            orion_qwen35_manifest_free(manifest);
            return 1;
        }
        ...
        printf("PASS: qwen35 9b lora pair training\n");
        printf("  blob_dir=%s\n", blob_dir);
        ...
        printf("  next_blocker=%s\n", "crpg mainline launch gate and ANE hybrid parity residual review");
