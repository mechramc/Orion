...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        ...
        if (!argmax_vocab_from_tied_embedding(blob_dir, last_hidden, d_model, manifest->vocab, &top_id, &top_logit)) {
            fprintf(stderr, "FAIL: failed vocab top1 scan\n");
            goto fail;
        }
        ...
        if (hidden_abs <= 0.0 || last_hidden_abs <= 0.0 || top_id < 0 || !isfinite(top_logit)) {
            fprintf(stderr, "FAIL: expected non-zero logits path\n");
            goto fail;
        }

        printf("PASS: qwen35 24layer logits cpu smoke\n");
        printf("  blob_dir=%s\n", blob_dir);
        printf("  seq_len=%d\n", seq_len);
        printf("  layers_run=%d\n", manifest->n_layer);
        printf("  hidden_abs_sum=%.6f\n", hidden_abs);
        printf("  last_hidden_abs_sum=%.6f\n", last_hidden_abs);
        printf("  top_token_id=%d\n", top_id);
        printf("  top_token_logit=%.6f\n", top_logit);
        printf("  next_blocker=%s\n", "decode loop and tokenizer parity validation");
